...
/** Default values for torque visualization */
export const TORQUE_CONSTANTS = {
  TORUS_RADIUS: 0.06,
  TUBE_RADIUS: 0.003,
  ARC_ANGLE: 270,
  LINE_OFFSET: 0.1,
  ...
  ARROW_HEAD_DIAMETER: 0.04,
  TORUS_SEGMENTS: 60,
  TORUS_TESSELLATION: 16,
} as const;
...
/**
 * Creates a torque visualization with incomplete torus arc and directional arrow.
...
export const createTorqueVisualization = (
  ...
  torusRadius: number = TORQUE_CONSTANTS.TORUS_RADIUS,
  tubeRadius: number = TORQUE_CONSTANTS.TUBE_RADIUS,
  arcAngle: number = TORQUE_CONSTANTS.ARC_ANGLE,
  lineOffset: number = TORQUE_CONSTANTS.LINE_OFFSET,
  lineLength: number = TORQUE_CONSTANTS.LINE_LENGTH,
  ...
  // Calculate perpendicular vectors for the torus plane
  const { perp1, perp2 } = calculatePerpendiculars(normalizedDirection);

  // Create torus arc
  const torus = createTorusArc(
    name,
    scene,
    centerPosition,
    perp1,
  ...
  // Create arrow at torus end
  const torusArrow = createTorusArrow(
    name,
    scene,
    centerPosition,
    perp1,
  ...
  // Create directional line and arrow
  const lineArrow = createDirectionalLine(
    name,
    scene,
    centerPosition,
    normalizedDirection,
  ...
  const lineArrowHead = createLineArrowHead(
    name,
    scene,
    centerPosition,
    normalizedDirection,
  ...
  // Create label
  const torqueLabel = createTorqueLabel(lineArrowHead);
...
/**
 * Creates a torus arc path in the plane perpendicular to a direction.
 * @private
 */
const createTorusArc = (
  name: string,
  scene: BABYLON.Scene,
  centerPosition: BABYLON.Vector3,
  ...
  for (let i = 0; i <= TORQUE_CONSTANTS.TORUS_SEGMENTS; i++) {
    const angle = (i / TORQUE_CONSTANTS.TORUS_SEGMENTS) * arcRadians;
    const y = torusRadius * Math.cos(angle);
    const z = torusRadius * Math.sin(angle);
    const point = centerPosition.add(perp1.scale(y)).add(perp2.scale(z));
    path.push(point);
  }
  ...
  const torus = BABYLON.MeshBuilder.CreateTube(name + 'IncompleteTorus', {
    path,
    radius: tubeRadius,
    tessellation: TORQUE_CONSTANTS.TORUS_TESSELLATION,
...
/**
 * Creates an arrow at the end of a torus arc.
 * @private
 */
const createTorusArrow = (
  name: string,
  scene: BABYLON.Scene,
  centerPosition: BABYLON.Vector3,
  perp1: BABYLON.Vector3,
  ...
  const tangentDirection = perp1.scale(Math.cos(tangentAngle)).add(perp2.scale(Math.sin(tangentAngle)));

  const arrow = BABYLON.MeshBuilder.CreateCylinder(name + 'TorusArrow', {
    diameterTop: 0,
    diameterBottom: TORQUE_CONSTANTS.ARROW_DIAMETER,
    height: TORQUE_CONSTANTS.ARROW_SIZE
  }, scene);

  arrow.position = centerPosition.add(perp1.scale(endY)).add(perp2.scale(endZ));
...
/**
 * Creates a directional line for torque visualization.
 * @private
 */
const createDirectionalLine = (
  name: string,
  scene: BABYLON.Scene,
  centerPosition: BABYLON.Vector3,
  ...
  const start = centerPosition.add(direction.scale(offset));
  const end = start.add(direction.scale(length));
  const distance = BABYLON.Vector3.Distance(start, end);

  const line = BABYLON.MeshBuilder.CreateCylinder(name + 'LineArrow', {
    diameter: 0.03,
    height: distance
  }, scene);

  line.position = BABYLON.Vector3.Lerp(start, end, 0.5);
  ...
  line.rotationQuaternion = rotation;
...
/**
 * Creates an arrow head at the end of a directional line.
 * @private
 */
const createLineArrowHead = (
  name: string,
  scene: BABYLON.Scene,
  centerPosition: BABYLON.Vector3,
  direction: BABYLON.Vector3,
  ...
  const end = centerPosition.add(direction.scale(offset + length));

  const arrowHead = BABYLON.MeshBuilder.CreateCylinder(name + 'LineArrowHead', {
    diameterTop: 0,
    diameterBottom: TORQUE_CONSTANTS.ARROW_HEAD_DIAMETER,
    height: TORQUE_CONSTANTS.ARROW_HEAD_SIZE
  }, scene);
  ...
  arrowHead.rotationQuaternion = rotation;
...
/**
 * Creates a GUI label for torque visualization.
 * @private
 */
const createTorqueLabel = (arrowHead: BABYLON.Mesh): GUI.TextBlock => {
