...
export const calculateRectanglePostPositions = (
  ...
  // Calculate spacing
  const spacingX =
    postCountX > 1 ? (columnWidth - 2 * postOffset) / (postCountX - 1) : 0;
  const spacingZ =
    postCountZ > 1 ? (columnDepth - 2 * postOffset) / (postCountZ - 1) : 0;
  ...
  for (let ix = 0; ix < postCountX; ix++) {
    for (let iz = 0; iz < postCountZ; iz++) {
      const x = postCountX > 1 ? -halfWidth + postOffset + ix * spacingX : 0;
      const z = postCountZ > 1 ? -halfDepth + postOffset + iz * spacingZ : 0;
